function parse(tokens) {
	...
	function define() {
		...
		next(); 
		return new StatementAt(nameToken.source, formals,
	...
	function relational() {
		var a = additive();
		while (has(MORE_OR_EQUAL) || has(MORE) || has(LESS_OR_EQUAL)
				|| has(LESS) || has(EQUAL) || has(NOT_EQUAL)) {
	...
	function atom() {
		if (has(INTEGER)) {
			...
			throw 'Error: I expected an expression, but instead I found \''	+ tokens[i].source + '\'. Check for missing attributes for keywords and/or misspelled keywords';
		}
	}
